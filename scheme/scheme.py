...
def eval_all(expressions, env):
    ...
    if expressions==nil:
        ...
        if expressions.second==nil:
            ...
            while isinstance(expressions.second,Pair):
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        value=do_lambda_form(Pair(expressions.first.second,expressions.second),env)
        env.define(name,value)
        return name
        # END Question 9A
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN Question 8
    "*** REPLACE THIS LINE ***"
    instance=LambdaProcedure(formals, expressions.second,env)
    return instance
    # END Question 8
...
def do_if_form(expressions, env):
    ...
    true_statement=expressions.second.first
    false_statement=expressions.second.second
    if scheme_true(pred):
        return scheme_eval(true_statement,env,True)
    elif false_statement != nil:
        return scheme_eval(false_statement.first,env,True)
    else:
        return okay
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        bind=bindings.first
        formal=Pair(bind.first,nil)
        check_form(bind,2,2)
        check_formals(formal)
        frame.define(bind.first,scheme_eval(bind.second.first,env))
...
def check_formals(formals):
    ...
    # BEGIN Question 11B
    "*** REPLACE THIS LINE ***"
    if isinstance(formals, Pair) and not scheme_listp(formals):
        raise SchemeError
    check_list=[]
    while formals is not nil:
        if formals.first in check_list or not scheme_symbolp(formals.first):
            raise SchemeError
        else:
            check_list.append(formals.first)
...
def do_mu_form(expressions, env):
    ...
    # BEGIN Question 17
    "*** REPLACE THIS LINE ***"
    instance=MuProcedure(formals,expressions.second)
    return instance
    # END Question 17
...
#Promise
    def evaluate(self):
        ...
        self.value=scheme_eval(self.expressions.first,new_env)
        self.evaluated=True
        return self.value
...
def do_cons_stream_form(expr,env):
    ...
    first=expr.first
    rest=expr.second
    return Pair(scheme_eval(first,env),Promise(rest,env))
